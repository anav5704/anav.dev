...
The full loop looks like this. Prisma pushes the schema out to Neon, Drizzle pulls it back in as typed models, and the app queries through Drizzle from then on.
...
flowchart TD
    ...
    B[Prisma db push]
    C[Neon Postgres]
    D[Drizzle introspect]
    E[Drizzle schema + relations]
    F[Next.js queries]

    A e1@-->|Define models| B
    B e2@-->|Migrate| C
    C e3@-->|Pull tables| D
    D e4@-->|Generate| E
    E e5@-->|Import| F
