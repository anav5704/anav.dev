...
That whole mapping table boils down to this flow. A visitor hits a subdomain, Vercel checks it against your host rules, and answers with a redirect or a 404.

```mermaid
flowchart TD
    A[/Visitor enters subdomain/]
    B[Vercel edge network]
    C{Host rule match?}
    D[Redirect to destination]
    E[404 not found]
